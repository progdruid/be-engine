...
auto BeTexture::Builder::SetBindFlags(BeBindFlags bindFlags)        -> Builder&& { _descriptor.BindFlags = bindFlags; return std::move(*this); }
auto BeTexture::Builder::SetFormat(BeTextureFormat format)          -> Builder&& { _descriptor.Format = format; return std::move(*this); }
...
auto BeTexture::Builder::Build(BeRenderer& renderer) -> std::shared_ptr<BeTexture> {
    std::shared_ptr<BeTexture> resource(new BeTexture(_descriptor));
    resource->CreatePlatformResources(renderer, _descriptor.Data);
    if (_addToRegistry)
        BeAssetRegistry::AddTexture(_descriptor.Name, resource);
    return resource;
}

auto BeTexture::Builder::BuildNoReturn(BeRenderer& renderer) -> void {
    std::shared_ptr<BeTexture> resource(new BeTexture(_descriptor));
    resource->CreatePlatformResources(renderer, _descriptor.Data);
    if (_addToRegistry)
        BeAssetRegistry::AddTexture(_descriptor.Name, resource);
